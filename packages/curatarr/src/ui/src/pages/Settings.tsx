...
import { ScoutTopSettingsContent } from '../components/settings/scout/ScoutTopSettingsContent';
...
function parseCustomCfRule(rule: {
  id: number;
  name: string;
  enabled: number;
  priority: number;
        ...
        <section className="space-y-4 py-3 border-t first:border-t-0" style={{ borderColor: 'var(--c-border)' }}>
            ...
            >
              ...
              >
                ...
                <span>
                  ...
                  compatibility warning for AV1 files when your selected primary client lacks hardware AV1 decode.
                  Active profile: <em style={{ color: 'var(--c-text)' }}>{activeProfile.label}</em> — Video codec AV1:{' '}
                  <span style={{ color: 'var(--c-text)' }}>{activeProfile.videoCodec.av1}</span>.
                </span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <Field
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {scoutSyncTrashMutation.isError && (
              <span className="text-xs text-red-400">{(scoutSyncTrashMutation.error as Error).message}</span>
            )}
          </div>
          <div
                          ...
                          {appliedRules.map((rule) => (
                            <tr key={rule.id} style={{ borderTop: '1px solid var(--c-border)' }}>
                              ...
                              <td
                                ...
                                style={{ color: rule.enabled ? '#4ade80' : '#f87171' }}
                              >
                                {rule.enabled ? 'Yes' : 'No'}
                              </td>
                              <td className="px-2 py-1">
          ...
          >
            ...
            <p className="text-xs" style={{ color: 'var(--c-muted)' }}>
              Add your own filename/title pattern rules (regex or string) and score deltas. These apply after TRaSH
              baseline scoring.
            </p>
            <div
            ...
            >
              ...
              {customCfPreviewMutation.data && (
                <div className="text-xs" style={{ color: 'var(--c-muted)' }}>
                  Delta:{' '}
                  <span style={{ color: 'var(--c-text)' }}>
                    {customCfPreviewMutation.data.delta >= 0 ? '+' : ''}
                    {customCfPreviewMutation.data.delta}
                  </span>{' '}
