...
import { AlertTriangle, File, FolderOpen, Loader2, Trash2, X } from 'lucide-react';
      ...
      >
        ...
        <div className="p-5 space-y-4">
          <p className="text-sm" style={{ color: 'var(--c-text)' }}>
            You are about to delete <strong>{movieTitle}</strong>. This is permanent and cannot be undone.
          </p>
          ...
          {isLoading ? (
            ...
            contents && (
              <div
                className="rounded-lg border overflow-hidden text-xs font-mono"
                style={{ borderColor: 'var(--c-border)', background: 'var(--c-bg)' }}
              >
                <div
                  className="px-3 py-2 text-xs font-sans font-medium border-b"
          ...
          {contents?.hasNonVideo && (
            ...
            >
              ...
              <span>
                ...
                {otherFiles.map((f) => f.name).join(', ')}). Choosing <strong>"Video files only"</strong> will leave
                these behind.
              </span>
            </div>
          )}
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <div>
                ...
                <div className="text-xs mt-0.5" style={{ color: 'var(--c-muted)' }}>
                  Delete {videoFiles.length} video file{videoFiles.length !== 1 ? 's' : ''}, keep the folder and any
                  other files.
                </div>
              </div>
            </label>
            ...
            >
              ...
              />
              <div>
                <div className="text-sm font-medium text-red-400">Entire folder</div>
                <div className="text-xs mt-0.5" style={{ color: 'var(--c-muted)' }}>
          ...
          {deleteMutation.data?.errors && deleteMutation.data.errors.length > 0 && (
            ...
            >
              {deleteMutation.data.errors.map((e) => (
                <div key={e}>{e}</div>
              ))}
            </div>
          )}
